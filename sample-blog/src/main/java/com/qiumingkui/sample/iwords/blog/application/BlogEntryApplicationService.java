...
import com.qiumingkui.sample.iwords.blog.domain.model.blogentry.BlogEntryPermissionPolicy;
...
public class BlogEntryApplicationService {
	...
	public String publishBlog(String aTitle, String aContent, Author aAuthor) throws Exception {
		BlogEntry blogEntry = BlogEntryBuilder.build(aTitle, aContent, aAuthor);
		...
		return blogEntry.blogEntryId().id();
	}
	 ...
	 */
	// @Transactional
	public BlogEntryData readBlog(String aBlogId, Reader aReader) throws Exception {
		...
		BlogEntryData blogEntryData = new BlogEntryData(blogEntry.blogEntryId().id(), blogEntry.title().titleTxt(), blogEntry.content().contentTxt(),
